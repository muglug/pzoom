...
use super::{
    node::{
        DataFlowNode, DataFlowNodeId, DataFlowNodeKind, FunctionLikeIdentifier, VariableSourceKind,
    },
...
pub struct DataFlowGraph {
    ...
    pub specializations: FxHashMap<DataFlowNodeId, FxHashSet<(StrId, u32)>>,
    specialized_calls: FxHashMap<(StrId, u32), FxHashSet<DataFlowNodeId>>,
}

impl Default for DataFlowGraph {
    fn default() -> Self {
        Self::new(GraphKind::FunctionBody)
    }
}
...
impl DataFlowGraph {
    ...
    pub fn add_path(
        ...
        path_kind: PathKind,
        added_taints: Vec<super::node::SinkType>,
        removed_taints: Vec<super::node::SinkType>,
    ) {
        if from_id == to_id {
            return;
        }
        ...
        for _ in 0..50 {
            ...
            for child_node_id in child_node_ids {
                ...
                if var_ids_only
                    && matches!(
                        child_node_id,
                        DataFlowNodeId::Var(..) | DataFlowNodeId::Param(..)
                    )
                {
    ...
    pub fn add_mixed_data(&mut self, assignment_node: &DataFlowNode, position_key: String) {
        ...
        for origin_node_id in origin_node_ids {
                ...
                origin_node_id
            {
                if let Some(entry) = self.mixed_source_counts.get_mut(&origin_node_id) {
                    entry.insert(position_key.clone());
                } else {
                    self.mixed_source_counts.insert(
                        origin_node_id.clone(),
                        FxHashSet::from_iter([position_key.clone()]),
                    );
                }
            }
        }
