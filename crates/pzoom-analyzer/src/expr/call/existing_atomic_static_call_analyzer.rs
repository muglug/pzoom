...
pub(crate) fn resolve_named_mixin_static_method(
    analyzer: &StatementsAnalyzer<'_>,
    class_info: &ClassLikeInfo,
    method_name: &str,
) -> Option<(
...
pub(crate) fn magic_call_handler_is_mutation_free(
    ...
    if get_method_info_case_insensitive(analyzer, class_info, method_name).is_some() {
        return false;
    }

    ["__callStatic", "__call"].iter().any(|handler| {
        get_method_info_case_insensitive(analyzer, class_info, handler)
...
pub(crate) fn get_pseudo_static_method_info_case_insensitive<'a>(
    analyzer: &StatementsAnalyzer<'_>,
    class_info: &'a ClassLikeInfo,
    method_name: &str,
) -> Option<&'a pzoom_code_info::FunctionLikeInfo> {
...
pub(crate) fn build_static_method_template_context(
    ...
    if let Some(invoking_class_info) = invoking_class_info {
            ...
            .get(&class_info.name)
        {
            function_call_analyzer::overlay_template_replacements(
                &mut template_replacements,
                invoking_template_map
                    .iter()
                    .map(|(template_name, replacement)| {
                        (*template_name, class_info.name, replacement.clone())
                    })
                    .collect(),
            );
        }
