...
use pzoom_code_info::codebase_info::{
    ConstantInfo, InlineCallableParamType, InlineCallableTypeAnnotation, InlineCheckTypeAnnotation,
...
impl<'a, 'p> DeclarationCollector<'a, 'p> {
    ...
    fn collect_inline_var_annotations_from_docblock(
        ...
        for content in var_tags.values() {
            ...
            let parsed_type = crate::docblock::parse_type_string(type_str, self.interner).unwrap_or_else(|_| TUnion::mixed());
        ...
        while let Some(start_rel) = self.source[cursor..].find("/**") {
            ...
            if comment.contains("@psalm-check-type") {
                ...
                let target_offset = self
                    .find_next_non_whitespace_offset(end as u32)
        ...
        for (key, is_exact) in [("psalm-check-type", false), ("psalm-check-type-exact", true)] {
            ...
            for content in tags.values() {
                ...
                let check_type = if rhs.is_empty() {
                    ...
                    let parsed_type = crate::docblock::parse_type_string(rhs, self.interner).unwrap_or_else(|_| TUnion::mixed());
    ...
    fn visit_constant(&mut self, constant: &Constant<'_>) {
        for item in &constant.items {
            ...
            let span = item.span();
            let constant_type =
                simple_type_inferer::infer(&item.value).unwrap_or_else(TUnion::mixed);
        ...
        for member in members {
            ...
            for item in &class_const.items {
                ...
                let constant_type = hinted_const_type
                    .clone()
                    .or_else(|| simple_type_inferer::infer(&item.value))
        ...
        for member in members {
            ...
            if let EnumCaseItem::Backed(backed_case) = &enum_case.item {
                let inferred_case_value =
                    simple_type_inferer::infer(&backed_case.value).unwrap_or_else(TUnion::mixed);
            ...
            match member {
                ClassLikeMember::Method(method) => {
                    ...
                    let mut is_external_mutation_free =
                        is_mutation_free || class_info.is_external_mutation_free;
    ...
    fn collect_required_classlikes_from_docblock_tags(
        ...
        for tag_key in tag_keys {
            ...
            for (_, content) in ordered_tags {
                ...
                let parsed_type = crate::docblock::parse_type_string(requirement, self.interner).unwrap_or_else(|_| TUnion::mixed());
                let resolved_type = self.resolve_docblock_union_type(
                    parsed_type,
                    self_class,
                    parent_class,
    ...
    fn apply_docblock_mixins(
        ...
        for (_, content) in ordered_tags {
            ...
            let parsed_type = crate::docblock::parse_type_string(mixin, self.interner).unwrap_or_else(|_| TUnion::mixed());
            let resolved_type = self.resolve_docblock_union_type(
                parsed_type,
                self_class,
                parent_class,
    ...
    fn parse_docblock_method_info(
        ...
        let return_type = if let Some(return_type_str) = return_type_str {
            let parsed_type =
                crate::docblock::parse_type_string(return_type_str.trim(), self.interner).unwrap_or_else(|_| TUnion::mixed());
        ...
        let parsed = split_docblock_method_params(params)
            ...
            .map(|(idx, raw_param)| {
                ...
                let type_union = if type_source.is_empty() {
                    ...
                    let parsed_type =
                        crate::docblock::parse_type_string(type_source, self.interner).unwrap_or_else(|_| TUnion::mixed());
                    Some(self.resolve_docblock_union_type(
                        parsed_type,
                        self_class,
                        parent_class,
    ...
    fn try_resolve_docblock_utility_type(
        ...
        let parsed_inner = crate::docblock::parse_type_string(inner, self.interner).unwrap_or_else(|_| TUnion::mixed());
        let resolved_inner =
    ...
    fn properties_of_or_deferred(
        ...
        self.resolve_properties_of_union(union, visibility)
    }
    ...
    fn parse_docblock_conditional_branch(
        ...
        if let Some(nested_conditional) = self.parse_docblock_conditional_return_type(
            ...
            return TUnion::new(TAtomic::TConditional(Box::new(nested_conditional)));
        }
        ...
        let parsed_branch_type = crate::docblock::parse_type_string(branch_type, self.interner).unwrap_or_else(|_| TUnion::mixed());
    ...
    fn parse_docblock_conditional_condition(
        ...
        // `$param is X` (parameter condition) — evaluated against the argument type at
        // the call site. The param name is interned with its `$` prefix, matching
        // ParamInfo::name.
        if lhs.starts_with('$') {
            let param_id = self.interner.intern(lhs);
            return Some(ConditionalReturnCondition::ParamIs {
                param_id,
                asserted_type,
            });
        }
    ...
    fn parse_assertion_tag_content(
            ...
            || assertion_source.eq_ignore_ascii_case("non-empty")
        {
            ...
            let parsed_type = crate::docblock::parse_type_string(assertion_source, self.interner).unwrap_or_else(|_| TUnion::mixed());
            let parsed_type = self.resolve_docblock_union_type(
                parsed_type,
                self_class,
                parent_class,
    ...
    fn collect_defined_constants_from_statements(
        ...
        for statement in statements {
            ...
            let constant_type =
                simple_type_inferer::infer(value_arg.value()).unwrap_or_else(TUnion::mixed);
    ...
    fn collect_docblock_type_aliases(
        ...
        for (_, content) in type_entries {
            ...
            let parsed_type = crate::docblock::parse_type_string(&type_definition, self.interner).unwrap_or_else(|_| TUnion::mixed());
            let resolved_type = self.resolve_docblock_union_type(
                parsed_type,
                self_class,
                parent_class,
        ...
        for (offset, content, variance) in template_entries {
            ...
            let as_type = if let Some(template_bound) = template_bound {
                self.try_resolve_template_key_of_type(&template_bound, Some(&template_map))
                    .unwrap_or_else(|| {
                        let parsed_type =
                            crate::docblock::parse_type_string(&template_bound, self.interner).unwrap_or_else(|_| TUnion::mixed());
                        self.resolve_docblock_union_type(
                            parsed_type,
                            self_class,
                            parent_class,
    ...
    fn is_valid_docblock_type_string(
        ...
        let parsed_type =
            match crate::docblock::parse_type_string_with_context(type_str, self.interner, &context)
            {
                Ok(parsed) => parsed,
                Err(_) => return false,
            };
