//! Variable-name string type, ported from Hakana's `code_info/var_name.rs`.
//!
//! A small-string-optimized newtype for variable ids (`$foo`, `$foo->bar`,
//! `$arr['key']`). Typical names fit inline in a `CompactString`, so cloning
//! and hashing them avoids heap traffic entirely — unlike `String` keys, and
//! without the lock/Arc overhead of routing variable names through the
//! interner.
