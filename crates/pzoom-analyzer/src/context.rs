...
use pzoom_code_info::{combine_union_types, TUnion};
use pzoom_str::StrId;
...
impl BlockContext {
            ...
            for (key, _) in &clause.possibilities {
                if let ClauseKey::Name(name) = key {
                    ...
                    for changed_var_id in changed_var_ids {
                        ...
                        if name == &*changed_name || name.starts_with(&format!("{}[", changed_name)) {
                            dominated = true;
    ...
    pub fn merge(&mut self, other: &BlockContext) {
        ...
        let common_assigned: FxHashSet<_> = self_keys
            .intersection(&other_keys)
            .copied()
            .collect();

        // Variables assigned in either branch are possibly assigned
        let all_assigned: FxHashSet<_> = self_keys
            .union(&other_keys)
            .copied()
            .collect();

        // Update assigned_var_ids to only have common assignments
        self.assigned_var_ids.retain(|k, _| common_assigned.contains(k));
